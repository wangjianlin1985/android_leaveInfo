...
public class TaskEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_state.getText().toString().equals("")) {
						Toast.makeText(TaskEditActivity.this, "当前状态输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_taskTime.getText().toString().equals("")) {
						Toast.makeText(TaskEditActivity.this, "创建时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传任务信息信息*/
					TaskEditActivity.this.setTitle("正在更新任务信息信息，稍等...");
	...
	private void initViewData() {
		...
		this.TV_taskId.setText(taskId+"");
		for (int i = 0; i < leaveInfoList.size(); i++) {
			if (task.getLeaveInfoObj() == leaveInfoList.get(i).getLeaveId()) {
				this.spinner_leaveInfoObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < noteList.size(); i++) {
			if (task.getNoteObj() == noteList.get(i).getNoteId()) {
				this.spinner_noteObj.setSelection(i);
				break;
			}
		}
		this.ET_state.setText(task.getState() + "");
		for (int i = 0; i < userInfoList.size(); i++) {
			if (task.getUserObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_userObj.setSelection(i);
				break;
			}
		}
		this.ET_taskTime.setText(task.getTaskTime());
	}
	@Override

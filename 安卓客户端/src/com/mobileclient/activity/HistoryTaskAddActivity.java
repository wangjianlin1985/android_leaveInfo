...
public class HistoryTaskAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.historytask_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加历史任务");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_checkSuggest.getText().toString().equals("")) {
						Toast.makeText(HistoryTaskAddActivity.this, "审批意见输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_taskTime.getText().toString().equals("")) {
						Toast.makeText(HistoryTaskAddActivity.this, "创建时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_checkState.getText().toString().equals("")) {
						Toast.makeText(HistoryTaskAddActivity.this, "审批状态输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传历史任务信息*/
					HistoryTaskAddActivity.this.setTitle("正在上传历史任务信息，稍等...");
					String result = historyTaskService.AddHistoryTask(historyTask);

...
public class NoteEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_noteName.getText().toString().equals("")) {
						Toast.makeText(NoteEditActivity.this, "节点名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_noteIndex.getText().toString().equals("")) {
						Toast.makeText(NoteEditActivity.this, "节点编号输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传节点信息信息*/
					NoteEditActivity.this.setTitle("正在更新节点信息信息，稍等...");
					String result = noteService.UpdateNote(note);
	...
	private void initViewData() {
		...
		this.TV_noteId.setText(noteId+"");
		this.ET_noteName.setText(note.getNoteName());
		for (int i = 0; i < departmentList.size(); i++) {
			if (note.getDepartmentObj() == departmentList.get(i).getDepartmentId()) {
				this.spinner_departmentObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < positionList.size(); i++) {
			if (note.getPositionObj() == positionList.get(i).getPositionId()) {
				this.spinner_positionObj.setSelection(i);
				break;
			}
		}
		this.ET_noteIndex.setText(note.getNoteIndex() + "");
	}
	@Override

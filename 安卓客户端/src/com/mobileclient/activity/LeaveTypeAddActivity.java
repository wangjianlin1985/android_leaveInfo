...
public class LeaveTypeAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.leavetype_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加请假类型");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_leaveTypeName.getText().toString().equals("")) {
						Toast.makeText(LeaveTypeAddActivity.this, "请假类型名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传请假类型信息*/
					LeaveTypeAddActivity.this.setTitle("正在上传请假类型信息，稍等...");
					String result = leaveTypeService.AddLeaveType(leaveType);

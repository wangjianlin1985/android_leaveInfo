...
public class LeaveInfoEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_writeTime.getText().toString().equals("")) {
						Toast.makeText(LeaveInfoEditActivity.this, "填表时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_sfcj.getText().toString().equals("")) {
						Toast.makeText(LeaveInfoEditActivity.this, "是否长假输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_leaveDays.getText().toString().equals("")) {
						Toast.makeText(LeaveInfoEditActivity.this, "请假天数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_place.getText().toString().equals("")) {
						Toast.makeText(LeaveInfoEditActivity.this, "前往地点输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_reason.getText().toString().equals("")) {
						Toast.makeText(LeaveInfoEditActivity.this, "请假事由输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_memo.getText().toString().equals("")) {
						Toast.makeText(LeaveInfoEditActivity.this, "备注输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_state.getText().toString().equals("")) {
						Toast.makeText(LeaveInfoEditActivity.this, "状态输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传请假信息信息*/
					LeaveInfoEditActivity.this.setTitle("正在更新请假信息信息，稍等...");
	...
	private void initViewData() {
		...
		this.ET_writeTime.setText(leaveInfo.getWriteTime());
		for (int i = 0; i < departmentList.size(); i++) {
			if (leaveInfo.getDepartmentObj() == departmentList.get(i).getDepartmentId()) {
				this.spinner_departmentObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < userInfoList.size(); i++) {
			if (leaveInfo.getUserObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_userObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < positionList.size(); i++) {
			if (leaveInfo.getPositionObj() == positionList.get(i).getPositionId()) {
				this.spinner_positionObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < leaveTypeList.size(); i++) {
			if (leaveInfo.getLeaveTypeObj() == leaveTypeList.get(i).getLeaveTypeId()) {
				this.spinner_leaveTypeObj.setSelection(i);
				break;
			}
		}
		this.ET_sfcj.setText(leaveInfo.getSfcj());
		...
		for (int i = 0; i < dayTimeTypeList.size(); i++) {
			if (leaveInfo.getStartDayTimeType() == dayTimeTypeList.get(i).getDayTimeTypeId()) {
				this.spinner_startDayTimeType.setSelection(i);
				break;
			}
		}
		...
		for (int i = 0; i < dayTimeTypeList.size(); i++) {
			if (leaveInfo.getEndDayTimeType() == dayTimeTypeList.get(i).getDayTimeTypeId()) {
				this.spinner_endDayTimeType.setSelection(i);
				break;
			}
		}
		this.ET_leaveDays.setText(leaveInfo.getLeaveDays() + "");
		for (int i = 0; i < userInfoList.size(); i++) {
			if (leaveInfo.getWriteUserObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_writeUserObj.setSelection(i);
				break;
			}
		}
		this.ET_place.setText(leaveInfo.getPlace());
		this.ET_reason.setText(leaveInfo.getReason());
		this.ET_memo.setText(leaveInfo.getMemo());
		this.ET_state.setText(leaveInfo.getState() + "");
	}
	@Override

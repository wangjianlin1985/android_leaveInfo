...
public class TaskQueryActivity extends Activity {
	...
	/*查询过滤条件保存到这个对象中*/
	private Task queryConditionTask = new Task();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.task_query);
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("设置任务信息查询条件");
		...
		leaveInfoObj_ShowText = new String[leaveInfoCount+1];
		leaveInfoObj_ShowText[0] = "不限制";
		for(int i=1;i<=leaveInfoCount;i++) { 
			leaveInfoObj_ShowText[i] = leaveInfoList.get(i-1).getLeaveId();
		} 
		...
		spinner_leaveInfoObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionTask.setLeaveInfoObj(leaveInfoList.get(arg2-1).getLeaveId()); 
				else
					queryConditionTask.setLeaveInfoObj(0);
			}
			@Override
		...
		spinner_noteObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionTask.setNoteObj(noteList.get(arg2-1).getNoteId()); 
				else
					queryConditionTask.setNoteObj(0);
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) {}
		});
		...
		spinner_userObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionTask.setUserObj(userInfoList.get(arg2-1).getUser_name()); 
				else
					queryConditionTask.setUserObj("");
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) {}
		});
		...
		btnQuery.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					/*获取查询参数*/
					queryConditionTask.setTaskTime(ET_taskTime.getText().toString());
					Intent intent = getIntent();
					//这里使用bundle绷带来传输数据
					Bundle bundle =new Bundle();
					//传输的内容仍然是键值对的形式
					bundle.putSerializable("queryConditionTask", queryConditionTask);

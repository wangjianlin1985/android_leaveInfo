...
public class NewsService {
	...
	public String AddNews(News news) {
		...
		params.put("publishDate", news.getPublishDate().toString());
	...
	public List<News> QueryNews(News queryConditionNews) throws Exception {
		...
		if(queryConditionNews != null) {
			...
			if(queryConditionNews.getPublishDate() != null) {
				urlString += "&publishDate=" + URLEncoder.encode(queryConditionNews.getPublishDate().toString(), "UTF-8");
			}
		}
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				news.setPublishDate(Timestamp.valueOf(object.getString("publishDate")));
				newsList.add(news);
			}
		} catch (Exception e) {
			e.printStackTrace();
